...
class LinkedList:
    ...
    def index(self, index):
        ...
        if index == 0:
            ...
            return None if self.next is None else self.next.index(index - 1)
    ...
    def size(self):
        ...
        """
        return 1 if self.next is None else 1 + self.next.size()
...
def perform_transaction(sender_number, receiver_number, amount):
    ...
    if sender_number not in users:
        print(f"Did not found the account with number: {sender_number}")
        return

    if receiver_number not in users:
        print(f"Did not found the account with number: {receiver_number}")
        return
...
def delete_account(account_number):
    ...
    if account_number not in users:
        print(f"Did not found the account with number: {account_number}")
        return
...
def beatify_field_name(field):
    if field == "full_name":
        return "Full Name"
    elif field == "account_creation_date":
